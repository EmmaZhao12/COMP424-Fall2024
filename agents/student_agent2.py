...
class SecondAgent(Agent):
  ...
  def step(self, chess_board, player, opponent):
    ...
    if len(valid_moves) > 20:         # Early game
      depth = board_size // 2        
      scale = 0.5
    elif len(valid_moves) > 10:       # Mid game
    ...
    while time.time() - start_time < time_limit:
      self._perform_simulations(root, player, opponent, depth=depth, start_time=start_time, time_limit=time_limit)
